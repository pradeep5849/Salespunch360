...
vi.mock("next/cache",()=>({revalidatePath:mocks.revalidate}));vi.mock("@/lib/auth/authorization",()=>({requirePermissionForMutation:mocks.role}));vi.mock("@/lib/db",()=>({db:{user:{findUnique:mocks.user}}}));vi.mock("@/lib/auth/email-verification",()=>({issueEmailVerification:mocks.issue,verifyEmailToken:mocks.verify}));vi.mock("@/lib/security/request",()=>({assertTrustedOrigin:mocks.origin,consumeRateLimit:mocks.limit}));
import{confirmVerification,resendVerificationEmail}from"./email-verification";
beforeEach(()=>{vi.clearAllMocks();mocks.role.mockResolvedValue({id:"admin",salesRole:"PRIMARY_ADMIN"});mocks.user.mockResolvedValue({email:"admin@example.com",emailVerifiedAt:null});mocks.limit.mockResolvedValue(true);mocks.issue.mockResolvedValue(undefined)});
describe("verification actions",()=>{
 it("uses the mutation-safe self-profile permission rather than legacy role authority",async()=>{await resendVerificationEmail({});expect(mocks.role).toHaveBeenCalledWith("PROFILE_SELF")});
