...
export async function applyAdvanceAction(form:FormData){await applyAdvance(payload(form));redirect(`/workspace/account/transactions/${JSON.parse(String(form.get("payload"))).documentId}`)}
export async function saveReminderAction(form:FormData){await schedulePaymentReminder(payload(form));revalidatePath(`/workspace/account/transactions/${JSON.parse(String(form.get("payload"))).documentId}`)}
