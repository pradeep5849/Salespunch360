...
export async function fieldCheckInAction(form:FormData):Promise<VisitActionResult>{const visitType=String(form.get("visitType"));try{const location={latitude:Number(form.get("latitude")),longitude:Number(form.get("longitude")),accuracyMeters:Number(form.get("accuracyMeters"))},common={visitType,location,visitNotes:String(form.get("visitNotes")||"")},input=visitType==="NEW"?{...common,name:String(form.get("name")),phone:String(form.get("phone")||"")}:visitType==="FOLLOW_UP"?{...common,leadId:String(form.get("leadId"))}:{...common,customerId:String(form.get("customerId"))};const photo=form.get("photo");await fieldCheckIn(input,photo instanceof File&&photo.size?photo:undefined);}catch(error){logFieldCheckInFailure(error,visitType);return{ok:false,error:fieldCheckInErrorMessage(error)};}revalidatePath("/workspace");revalidatePath("/workspace/check-ins");revalidatePath("/workspace/leads");return{ok:true};}
