...
export async function replaceCompanyLogo(_:CompanyProfileState,formData:FormData):Promise<CompanyProfileState>{try{const actor=await requirePermission("SALES_SETTINGS");if(!actor.companyId)throw new Error("NOT_FOUND");const file=formData.get("companyLogo");if(!(file instanceof File)||!file.size)throw new Error("LOGO_INVALID");const data=await processCompanyLogo(file),key=companyLogoKey(actor.companyId);await privateStorage().put(key,data);await db.company.update({where:{id:actor.companyId},data:{logoObjectKey:key}});revalidatePath("/workspace");revalidatePath("/workspace/company-profile");return{success:"Company logo updated."};}catch(error){return{error:error instanceof Error&&error.message==="LOGO_INVALID"?"Choose a valid JPEG, PNG, or WebP image up to 5 MB.":"Unable to update the company logo."};}}
