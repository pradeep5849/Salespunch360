...
function user(role = "COMPANY_ADMIN", companyId: string | null = "company", isActive = true, salesRole:string|null=role==="SUPER_ADMIN"?null:"PRIMARY_ADMIN", managerType:string|null=null) { return { id: "user", role, companyId, isActive, passwordHash: "hash", salesRole, managerType, salesAccessActive:role!=="SUPER_ADMIN", accountRole:null, accountAccessActive:false }; }
...
describe("authoritative sign-in destination", () => {
  ...
  it("blocks Sales employees from web sign in", async () => { mocks.find.mockResolvedValue(user("COMPANY_ADMIN","company",true,"SALES")); await expect(signIn({},data())).resolves.toEqual({error:"This account can sign in only in the SalesPunch360 mobile app."}); expect(mocks.session).not.toHaveBeenCalled(); });
  it("blocks Sales Managers from web sign in", async () => { mocks.find.mockResolvedValue(user("COMPANY_ADMIN","company",true,"MANAGER","FIELD_MANAGER")); await expect(signIn({},data())).resolves.toEqual({error:"This account can sign in only in the SalesPunch360 mobile app."}); expect(mocks.session).not.toHaveBeenCalled(); });
  it("allows Office Managers to use web sign in", async () => { mocks.find.mockResolvedValue(user("COMPANY_ADMIN","company",true,"MANAGER","MANAGER_ONLY")); await signIn({},data()); expect(mocks.session).toHaveBeenCalledWith("user",false,"hash"); expect(mocks.redirect).toHaveBeenCalledWith("/workspace"); });
