...
function failure(error:unknown){if(error instanceof MobileEmployeeError)return mobileJson({error:error.code},error.status);const code=error instanceof Error?error.message:'';if(code==='MOBILE_UNAUTHORIZED')return mobileJson({error:'UNAUTHORIZED'},401);if(['SEAT_LIMIT','LIFECYCLE_BLOCKED','MANAGERS_DISABLED','INVALID_MANAGER','EMAIL_VERIFICATION_REQUIRED','COMPANY_PROFILE_REQUIRED'].includes(code))return mobileJson({error:code},409);return mobileJson({error:'INVALID_INPUT'},400)}
