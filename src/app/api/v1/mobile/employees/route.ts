...
function failure(error:unknown){if(error instanceof MobileEmployeeError)return mobileJson({error:error.code},error.status);const unauthorized=mobileUnauthorized(error);if(unauthorized)return unauthorized;const code=error instanceof Error?error.message:'';if(['SEAT_LIMIT','LIFECYCLE_BLOCKED','MANAGERS_DISABLED','INVALID_MANAGER','EMAIL_VERIFICATION_REQUIRED','COMPANY_PROFILE_REQUIRED'].includes(code))return mobileJson({error:code},409);if(error instanceof SyntaxError||error instanceof ZodError)return mobileJson({error:'INVALID_INPUT'},400);return mobileUnexpected('MOBILE_EMPLOYEES',error)}
