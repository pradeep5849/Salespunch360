...
export async function POST(request:Request){
 try{const user=await authenticateMobileSalesToken(request.headers.get('authorization'));const body=await request.json() as {action?:unknown;taskId?:unknown;outcomeNote?:unknown};if(typeof body.taskId!=='string'||!body.taskId)return mobileJson({error:'INVALID_REQUEST'},400);if(body.action==='CANCEL')await cancelFollowUpTaskForActor(user,body.taskId);else if(body.action==='COMPLETE_CALL')await completeCallFollowUpTaskForActor(user,body.taskId,body.outcomeNote);else return mobileJson({error:'INVALID_REQUEST'},400);return mobileJson({ok:true});}
