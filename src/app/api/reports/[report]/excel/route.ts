...
import { reportActor, type ReportActor } from "@/lib/reports/scope";
import { listTargetsForActor } from "@/lib/targets/service";
...
export async function dataFor(report:string,raw:Record<string,string>,providedActor?:ReportActor):Promise<ExportData>{
 ...
 else if(report==="expenses"){const r=await expenseReport(raw,actor);if(r.rows.length>MAX_ROWS||r.pointLimitExceeded)throw new Error("ROW_LIMIT");data={name:"Expense Report",range:`${r.filters.startText} to ${r.filters.endText}`,summary:{rows:r.rows.length,approved:r.rows.filter(x=>x.status==="APPROVED").length,pending:r.rows.filter(x=>x.status==="PENDING").length,rejected:r.rows.filter(x=>x.status==="REJECTED").length},headers:["Employee","Date","Daily distance km","Rate / km","Calculated amount","Approval mode","Status","Approved amount"],rows:r.rows.map(x=>[x.employee,x.date,x.distanceMeters/1000,x.ratePerKm?.toNumber()??null,x.amount?.toNumber()??null,x.approvalMode,x.status??"RATE REQUIRED",x.approvedAmount?.toNumber()??null])}}
 else throw new Error("NOT_FOUND");return{...data,companyId:actor.companyId};
}

export async function excelResponse(report:string,raw:Record<string,string>,providedActor?:ReportActor){const d=await dataFor(report,raw,providedActor),company=await db.company.findUniqueOrThrow({where:{id:d.companyId},select:{name:true}}),workbook=new ExcelJS.Workbook(),sheet=workbook.addWorksheet("Report");sheet.addRow(["Company",company.name]);sheet.addRow(["Report",d.name]);sheet.addRow(["Selected date range",d.range]);sheet.addRow(["Active filters",Object.entries(raw).filter(([k,v])=>k!=="page"&&v).map(([k,v])=>`${k}: ${v}`).join(", ")||"None"]);sheet.addRow(["Generated",india(new Date())]);for(const[key,value]of Object.entries(d.summary))sheet.addRow([key,typeof value==="object"?JSON.stringify(value):value]);sheet.addRow([]);const header=sheet.addRow(d.headers);header.font={bold:true};for(const row of d.rows)sheet.addRow(row);sheet.columns.forEach(c=>{c.width=18});const buffer=await workbook.xlsx.writeBuffer();return new Response(new Uint8Array(buffer),{headers:{"Content-Type":"application/vnd.openxmlformats-officedocument.spreadsheetml.sheet","Content-Disposition":`attachment; filename="${report}.xlsx"`,"Cache-Control":"private, no-store"}})}

export async function GET(request:Request,{params}:{params:Promise<{report:string}>}){try{const{report}=await params;return await excelResponse(report,Object.fromEntries(new URL(request.url).searchParams))}catch(error){const limited=error instanceof Error&&error.message==="ROW_LIMIT";return Response.json({error:limited?"More than 10,000 rows match. Narrow your filters and try again.":"Unable to export this report."},{status:limited?422:404,headers:{"Cache-Control":"no-store"}})}}