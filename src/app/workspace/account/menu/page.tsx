...
export default async function Page() {
  ...
  return <div className="account-menu-page">
    ...
    {navigation.map(group => <section className="account-menu-section" key={group.label} id={group.label.toLowerCase().replaceAll(" ", "-")}><h2>{group.label}</h2>{group.expandable ? <details><summary><span className="account-menu-icon" aria-hidden>{group.label === "Sales" ? "↗" : "↙"}</span>{group.label}<i>⌄</i></summary><nav>{group.items.map(item => <Link href={item.href} key={item.href + item.label}>{item.label}<span>›</span></Link>)}</nav></details> : <nav>{group.items.map(item => <Link href={item.href} key={item.href + item.label}><span className="account-menu-icon" aria-hidden>•</span>{item.label}<i>›</i></Link>)}</nav>}</section>)}
