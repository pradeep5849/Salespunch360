...
describe("check-in UI structure",()=>{
 ...
 it("uses the shared header without a duplicate SP badge",()=>{expect(page).not.toContain('<span>SP</span>');expect(page).not.toContain('SalesPunch360</div>');expect(page).toContain('WorkspacePageHeader title="Check-ins" backHref="/workspace"');});
