...
export function BillingCalculator({managersEnabled,adminUsage,managerUsage,salesUsage,telecallerUsage=0,telecallerLimit=0,prices,isRenewal,activeEmployees,isPlus=false,accountPackages=1,accountPrices={SIX_MONTH:400,YEARLY:700},initialAdminSeats,initialManagerSeats,initialSalesSeats}:{managersEnabled:boolean;adminUsage:number;managerUsage:number;salesUsage:number;telecallerUsage?:number;telecallerLimit?:number;prices:Prices;isRenewal:boolean;activeEmployees:Employee[];isPlus?:boolean;accountPackages?:number;accountPrices?:AccountPrices;initialAdminSeats?:number;initialManagerSeats?:number;initialSalesSeats?:number}){
 ...
 const validateRetention=(event:FormEvent<HTMLFormElement>)=>{
  setSelectionError('');
  if(telecaller<telecallerUsage){event.preventDefault();setSelectionError('Telecaller seats cannot be below the number of active Telecallers. Deactivate employees first, then renew.');return}
