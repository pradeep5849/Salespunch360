...
const telecallerPrices:Record<Period,number>={SIX_MONTH:600,YEARLY:1000};
...
export function AddTeamCalculator({managersEnabled,adminLimit,managerLimit,salesLimit,telecallerLimit=0,period,prices,isPlus=false,accountPackages=1,endsAt}:{managersEnabled:boolean;adminLimit:number;managerLimit:number;salesLimit:number;telecallerLimit?:number;period:Period;prices:Prices;isPlus?:boolean;accountPackages?:number;endsAt?:Date|null}){
 ...
 return <form action="/workspace/billing/checkout" method="GET" className="billing-form billing-calculator">
  ...
  <p className="muted">Add Admin, Manager, Sales or Telecaller seats without renewing. Only added seats are charged for the remaining time and every Sales-team seat keeps the same expiry date.</p>
...
export function BillingCalculator({managersEnabled,adminUsage,managerUsage,salesUsage,telecallerUsage=0,prices,isRenewal,activeEmployees,isPlus=false,accountPackages=1,accountPrices={SIX_MONTH:400,YEARLY:700},initialAdminSeats,initialManagerSeats,initialSalesSeats,initialTelecallerSeats=0}:{managersEnabled:boolean;adminUsage:number;managerUsage:number;salesUsage:number;telecallerUsage?:number;prices:Prices;isRenewal:boolean;activeEmployees:Employee[];isPlus?:boolean;accountPackages?:number;accountPrices?:AccountPrices;initialAdminSeats?:number;initialManagerSeats?:number;initialSalesSeats?:number;initialTelecallerSeats?:number}){
 ...
 const[telecaller,setTelecaller]=useState(initialTelecallerSeats);
 ...
 const salesSubtotal=admin*p.admin+manager*p.manager+sales*p.sales+telecaller*telecallerPrices[period];
 ...
 const telecallerBelowUsage=telecaller<telecallerUsage;
 const validateRetention=(event:FormEvent<HTMLFormElement>)=>{
  setSelectionError('');
  if(initialReduction||telecallerBelowUsage){event.preventDefault();setSelectionError('Selected seats cannot be below current active users. Deactivate users first or keep enough seats for the active team.');return}
  if(!isRenewal)return;
 ...
 return <form action="/workspace/billing/checkout" method="GET" className={`billing-form billing-calculator ${isPlus?styles.plusForm:''}`} onSubmit={validateRetention}>
  ...
  {isPlus?<>
   ...
   <label>Telecaller Seats<input name="telecallerSeats" type="number" min="0" max="10000" value={telecaller} onChange={e=>setTelecaller(clamp(Number(e.target.value)))}/><span>{money(telecallerPrices[period])} each</span></label>
  </>}
  {isRenewal&&admin<adminUsage&&<Retain role="ADMIN" count={admin} employees={activeEmployees}/>} {isRenewal&&manager<managerUsage&&<Retain role="MANAGER" count={manager} employees={activeEmployees}/>} {isRenewal&&sales<salesUsage&&<Retain role="SALES" count={sales} employees={activeEmployees}/>}<p className="muted">{isRenewal?'Renew Admin, Manager, Sales and Telecaller seats together on one common term. If you only need more seats during the current term, use Add Team instead.':'Choose the starting team for the new subscription.'}</p><div className="billing-total"><span>Sales Team Subtotal</span><strong>{money(salesSubtotal)}</strong></div>{isPlus&&<div className="billing-total"><span>Account Subtotal</span><strong>{money(accountSubtotal)}</strong></div>}<div className="billing-total"><span>Total</span><strong>{money(total)}</strong></div>{telecallerBelowUsage&&<p className="form-error">Telecaller seats cannot be below the current active Telecallers. Deactivate Telecallers first to reduce the next renewal quantity.</p>}{!hasSalesProduct&&<p className="form-error">Choose at least one Manager or Sales seat to keep Sales active. With no Manager/Sales seats, renew as Account-only.</p>}{selectionError&&<p className="form-error">{selectionError}</p>}<button disabled={!hasSalesProduct||initialReduction||telecallerBelowUsage}>Continue to Checkout</button>
 </form>;
}
