...
const eSafe=(e:Awaited<ReturnType<typeof billingDashboard>>['entitlement'])=>e;
export default async function Page({searchParams}:{searchParams:Search}){
 ...
 const salesFull=admin*price('ADMIN')+manager*price('MANAGER')+sales*price('SALES');
 const isPlus=r.company?.productEdition==='SALESPUNCH360_PLUS',accountPackages=isPlus?Math.max(1,count(q.accountPackages)||1):0;
 const accountUnit=isPlus?Number(r.prices.find(p=>p.role==='ACCOUNT_PACKAGE'&&p.period===period)?.amount??(period==='SIX_MONTH'?ACCOUNT_PACKAGE_SIX_MONTH_PRICE_INR:ACCOUNT_PACKAGE_YEARLY_PRICE_INR)):0,accountFull=accountPackages*accountUnit,total=salesFull+accountFull;
