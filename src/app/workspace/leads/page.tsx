...
export default async function LeadsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
 ...
 return <main className="leads-shell"><section className="leads-content"><WorkspacePageHeader title="Leads" backHref="/workspace"/>
  {user.salesRole!=="SALES"&&<form className="lead-scope-filter"><input name="q" defaultValue={q.q} placeholder="Search name or phone"/><select name="assignedUserId" defaultValue={q.assignedUserId||""}><option value="">All Employees</option>{scopeOptions.map(x=><option value={x.id} key={x.id}>{x.name} · {x.role==="MANAGER"?(x.managerType==="MANAGER_ONLY"?"Manager Only":"Field Manager"):"Sales"}</option>)}</select><button>View</button></form>}
