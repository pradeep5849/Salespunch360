...
export default async function LeadsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
 ...
 return <main className="leads-shell"><section className="leads-content"><WorkspacePageHeader title="Leads" backHref="/workspace"/>
  ...
  {!isPending&&<>{(user.role!=="SALES"||q.visitId)&&<LeadForm users={o.users} customers={o.customers} visitId={q.visitId}/>}<form className="lead-filters"><input name="q" defaultValue={q.q} placeholder="Search lead, customer, prospect"/><select name="stage" defaultValue={q.stage||""}><option value="">All stages</option>{pipeline.map(x=><option value={x.stage} key={x.stage}>{x.label}</option>)}</select>{user.role!=="SALES"&&<select name="assignedUserId" defaultValue={q.assignedUserId||""}><option value="">All permitted assignees</option>{o.users.map(x=><option value={x.id} key={x.id}>{x.name}</option>)}</select>}<button>Filter</button></form></>}
