...
import {leadOptions,leadScopeOptions,leadStageCounts,listLeads,listPendingVisitsScopedForActor,pendingVisitCountScopedForActor} from "@/lib/leads/service";
...
export default async function LeadsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
 ...
 const[leads,o,scopeOptions,counts,pending,pendingCount]=await Promise.all([listLeads(filters),leadOptions(),leadScopeOptions(),leadStageCounts(filters.assignedUserId,filters.q),isPending?listPendingVisitsScopedForActor(actor,filters.assignedUserId,filters.q):Promise.resolve([]),pendingVisitCountScopedForActor(actor,filters.assignedUserId,filters.q)]);
