...
import {addPhoneToVisitAction} from "@/app/actions/visits";
...
export default async function LeadsPage({searchParams}:{searchParams:Promise<Record<string,string|undefined>>}){
 ...
 const[leads,o,counts,pending,pendingCount]=await Promise.all([listLeads(filters),leadOptions(),leadStageCounts(),isPending?listPendingVisitsForActor(actor):Promise.resolve([]),pendingVisitCountForActor(actor)]);
 const submitPhone=async(form:FormData)=>{"use server";await addPhoneToVisitAction(form);};
 ...
 return <main className="leads-shell"><header className="employees-header"><Link href="/workspace">← Workspace</Link><div className="logo"><span>SP</span> SalesPunch360</div></header><section className="leads-content"><p className="eyebrow">Tenant-secure sales pipeline</p><h1>Leads</h1>
  <div className="lead-stage-summary"><Link className={!isPending&&!filters.stage?"active":""} href="/workspace/leads"><strong>{total}</strong><span>All Leads</span></Link><Link className={isPending?"active":""} href="/workspace/leads?view=pending"><strong>{pendingCount}</strong><span>Pending</span></Link>{pipeline.map(x=><Link className={!isPending&&filters.stage===x.stage?"active":""} href={`/workspace/leads?stage=${x.stage}`} key={x.stage}><strong>{counts[x.stage]??0}</strong><span>{x.label}</span></Link>)}</div>
  {!isPending&&<>{(user.role!=="SALES"||q.visitId)&&<LeadForm users={o.users} customers={o.customers} visitId={q.visitId}/>}<form className="lead-filters"><input name="q" defaultValue={q.q} placeholder="Search lead, customer, prospect"/><select name="stage" defaultValue={q.stage||""}><option value="">All stages</option>{pipeline.map(x=><option value={x.stage} key={x.stage}>{x.label}</option>)}</select>{user.role!=="SALES"&&<select name="assignedUserId" defaultValue={q.assignedUserId||""}><option value="">All permitted assignees</option>{o.users.map(x=><option value={x.id} key={x.id}>{x.name}</option>)}</select>}<select name="followUp" defaultValue={q.followUp||""}><option value="">Any follow-up</option><option value="overdue">Overdue</option><option value="today">Due today</option><option value="upcoming">Upcoming</option></select><button>Filter</button></form></>}
  {isPending?<div className="pipeline"><section><h2>Pending <span>{pendingCount}</span></h2>{pending.length===0&&<p className="pending-empty">No check-ins are waiting for a phone number.</p>}{pending.map(v=><article key={v.id}><strong>{v.contactName||v.customer?.name||"Field prospect"}</strong><span>{v.user.name} · {v.checkedInAt.toLocaleString()}</span><span>{v.checkedOutAt?`Checked out ${v.checkedOutAt.toLocaleString()}`:"Checkout pending"}</span>{v.visitNotes&&<span>{v.visitNotes}</span>}<small>{v.customer?.address||`${v.checkInLatitude.toFixed(5)}, ${v.checkInLongitude.toFixed(5)}`}</small>{v.photo&&<Link href={`/api/visit-photos/${v.id}`}>View photo / details</Link>}{v.user.id===user.id?<form action={submitPhone}><input type="hidden" name="visitId" value={v.id}/><input name="phone" required inputMode="tel" placeholder="Add phone to create Lead"/><button>Add phone</button></form>:<Link href="/workspace/check-ins">View check-in details</Link>}</article>)}</section></div>:
  <div className="pipeline">{(filters.stage?pipeline.filter(x=>x.stage===filters.stage):pipeline).map(({stage,label})=><section key={stage}><h2>{label} <span>{counts[stage]??0}</span></h2>{leads.filter(l=>l.stage===stage).map(l=>{const visitCount=leadVisitCount(l);return <article key={l.id}><Link href={`/workspace/leads/${l.id}`}><strong>{l.title}</strong></Link><span>{l.customer?.name||l.companyName||"Prospect"}</span><span>{l.assignedUser.name} · {l.source}</span><b>{visitCount} check-in / visit{visitCount===1?"":"s"}</b>{l.estimatedValue&&<b>{l.currencyCode} {l.estimatedValue.toFixed(2)}</b>}<span>{l.followUpAt?`Follow up ${l.followUpAt.toLocaleDateString("en-IN",{timeZone:"Asia/Kolkata"})}`:"No follow-up"}</span><Link href={`/workspace/leads/${l.id}#add-follow-up`}>+ Add Follow-up</Link><small>Updated {l.updatedAt.toLocaleDateString()}</small>{user.role!=="COMPANY_ADMIN"&&l.assignedUser.id===user.id&&<Link className="add-checkin" href={`/workspace/check-ins?leadId=${l.id}`}>+ Add Check-in</Link>}{(user.role==="COMPANY_ADMIN"||l.assignedUser.id===user.id)&&<DeleteLead leadId={l.id}/>}<StageForm id={l.id} version={l.version} stage={stage}/></article>})}</section>)}</div>}
 </section></main>;
}
