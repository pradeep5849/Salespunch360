...
export default async function CustomersPage({ searchParams }: { searchParams: Promise<{ q?: string }> }) {
  const user = await requireUser(); const q = (await searchParams).q ?? ""; const customers = await searchCustomers(q); const assignees=user.role==="COMPANY_ADMIN"&&user.companyId?await db.user.findMany({where:{companyId:user.companyId,isActive:true,OR:[{role:"SALES"},{role:"MANAGER",managerType:"FIELD_MANAGER"}]},select:{id:true,name:true,role:true},orderBy:{name:"asc"}}):[];
