...
export default async function AdminLayout({ children }: { children: ReactNode }) {
  ...
  return <div className="admin-shell"><header className="admin-header">
    ...
    <nav aria-label="Super Admin navigation"><Link href="/admin">Dashboard</Link><Link href="/admin/companies">Companies</Link><Link href="/admin/pricing">Pricing</Link><Link href="/admin/testimonials">Testimonials</Link><Link href="/admin/blog">Blog</Link><Link href="/admin/billing/orders">Billing</Link><form action={signOut}><button type="submit">Sign out</button></form></nav>
  </header>{children}</div>;
}
