...
export async function updatePublicSiteSettingsAction(fd:FormData){await requireGlobalSuperAdmin();const data={facebookUrl:url(fd.get("facebookUrl")),instagramUrl:url(fd.get("instagramUrl")),linkedinUrl:url(fd.get("linkedinUrl")),youtubeUrl:url(fd.get("youtubeUrl")),googlePlayUrl:url(fd.get("googlePlayUrl"))};await db.publicSiteSettings.upsert({where:{id:"default"},create:{id:"default",...data},update:data});revalidatePath("/");revalidatePath("/admin/public-site")}
