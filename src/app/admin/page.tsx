...
export default async function AdminDashboardPage() {
  ...
  const { counts, billing, recentCompanies } = await getAdminDashboard();
  ...
  return <main className="admin-content">
    <div className="admin-title-row"><div><p className="eyebrow">Platform administration</p><h1>Dashboard</h1><p className="muted">A safe operational overview of registered Companies.</p></div><div className="admin-actions"><Link href="/admin/companies">View Companies</Link><Link href="/admin/billing/orders">Billing</Link></div></div>
    <section className="admin-summary" aria-label="Company status summary">{cards.map(([label, count]) => <article key={label}><span>{label}</span><strong>{count}</strong></article>)}</section><section className="admin-summary" aria-label="Billing summary"><article><span>Pending Payments</span><strong>{billing.pendingOrders}</strong></article><article><span>Captured Payments</span><strong>{billing.capturedPayments}</strong></article><article><span>Captured Revenue</span><strong>₹{billing.capturedRevenue.toLocaleString('en-IN')}</strong></article><article><span>Active Users</span><strong>{billing.activeUsers}</strong></article></section>
