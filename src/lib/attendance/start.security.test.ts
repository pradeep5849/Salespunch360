...
vi.mock("@/lib/auth/authorization",()=>({requirePermissionForMutation:mocks.role,requirePermission:mocks.role}));vi.mock("@/lib/billing/entitlement",()=>({assertOperationalWrite:mocks.entitlement}));vi.mock("@/lib/db",()=>({db:{$transaction:mocks.transaction,geofenceEvent:{create:vi.fn()}}}));
import{startAttendance}from"./service";
beforeEach(()=>{vi.clearAllMocks();mocks.role.mockResolvedValue({id:"sales",role:"SALES",salesRole:"SALES",companyId:"company"});mocks.user.mockResolvedValue({id:"sales"});mocks.company.mockResolvedValue({attendanceEnabled:true,gpsTrackingEnabled:true,attendanceGeofenceEnabled:false,attendanceReferenceLatitude:null,attendanceReferenceLongitude:null,attendanceGeofenceRadiusMeters:null});mocks.attendance.mockResolvedValue(null);mocks.create.mockResolvedValue({id:"attendance"});mocks.transaction.mockImplementation(async(fn)=>fn({$queryRaw:mocks.query,user:{findFirst:mocks.user},company:{findFirst:mocks.company},attendance:{findFirst:mocks.attendance,create:mocks.create}}))});
