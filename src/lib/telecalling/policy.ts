...
export const isTelecaller=(user:Pick<AuthenticatedUser,"salesRole"|"designation">)=>user.salesRole==="SALES"&&isTelecallerDesignation(user.designation);
...
export function telecallingLeadScope(user:{id:string;salesRole:string|null;managerType?:string|null;designation?:string|null}){
 if(isTelecallerDesignation(user.designation)&&user.salesRole==="SALES")return {};
