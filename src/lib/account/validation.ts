...
export const partySchema=z.object({name:z.string().trim().min(1).max(200),contactPerson:optional(160),phone:optional(30),email:z.preprocess(v=>typeof v==="string"&&!v.trim()?undefined:v,z.string().trim().email().max(320).transform(v=>v.toLowerCase()).optional()),address:optional(),shippingAddress:optional(),gstin:optionalBusinessId(/^\d{2}[A-Z]{5}\d{4}[A-Z][1-9A-Z]Z[0-9A-Z]$/,"Enter a valid GSTIN"),stateCode:z.preprocess(v=>typeof v==="string"&&!v.trim()?undefined:v,z.string().regex(/^\d{2}$/).optional()),gstRegistrationType:z.enum(["UNREGISTERED","REGULAR","COMPOSITION","SEZ"]).default("UNREGISTERED"),pan:optionalBusinessId(/^[A-Z]{5}\d{4}[A-Z]$/,"Enter a valid PAN"),notes:optional()});
export const itemSchema=z.object({name:z.string().trim().min(1).max(200),code,categoryId:id.optional(),unitId:id.optional(),description:optional(),sellingRate:money,cost:money,taxRate:z.preprocess(v=>v===""||v==null?undefined:v,z.coerce.number().min(0).max(100).optional()),hsnSacCode:optional(12)});
