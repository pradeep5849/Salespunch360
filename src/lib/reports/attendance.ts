...
export async function attendanceReport(raw:SearchParams,providedActor?:ReportActor,exportMode=false){
 const actor=providedActor??await reportActor(),filters=parseReportFilters(raw,undefined,exportMode?10000:undefined),userIds=await resolveEmployeeScope(actor,filters.employeeId),branches=await operationalBranchContext(actor,typeof raw.branchId==="string"?raw.branchId:undefined);const status=raw.status==="COMPLETED"||raw.status==="OPEN"?raw.status:"ALL";
