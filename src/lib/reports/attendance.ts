...
export async function attendanceReport(raw:SearchParams){
 ...
 const rows=records.map(a=>{const route=orderedRoute(a.locationPoints);return {...a,durationMs:durationMs(a.startedAt,a.endedAt),routeDistanceMeters:calculateRouteDistanceMeters(route)}});const totalDurationMs=aggregates.reduce((n,a)=>n+(durationMs(a.startedAt,a.endedAt)||0),0);
