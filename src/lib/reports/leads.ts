...
export async function leadReport(raw:SearchParams,providedActor?:ReportActor,exportMode=false){
 const actor=providedActor??await reportActor(),filters=parseReportFilters(raw,undefined,exportMode?10000:undefined),ids=await resolveEmployeeScope(actor,filters.employeeId),branches=await operationalBranchContext(actor,typeof raw.branchId==="string"?raw.branchId:undefined),stage=stages.includes(raw.stage as LeadStage)?raw.stage as LeadStage:undefined,source=sources.includes(raw.source as LeadSource)?raw.source as LeadSource:undefined,follow=typeof raw.followUp==="string"?raw.followUp:"ALL";
