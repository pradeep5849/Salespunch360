...
export async function leadReport(raw:SearchParams){
 ...
 const [total,all,rows,employees]=await Promise.all([db.lead.count({where}),db.lead.findMany({where,select:{stage:true,estimatedValue:true,sourceVisitId:true,followUpAt:true},take:10001}),db.lead.findMany({where,include:{assignedUser:{select:{name:true,role:true}},customer:{select:{name:true}}},orderBy:[{createdAt:"desc"},{id:"desc"}],skip:(filters.page-1)*filters.pageSize,take:filters.pageSize}),reportEmployeeOptions(actor)]);if(all.length>10000)throw new Error("Report summary exceeds the interactive record limit; narrow the filters.");
