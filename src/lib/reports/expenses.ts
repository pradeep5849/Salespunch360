...
export async function expenseReport(raw:SearchParams,provided?:ReportActor){
 ...
 const daily=new Map<string,{branchId:string;employeeId:string;employee:string;date:string;distanceMeters:number;rate:Prisma.Decimal|null}>();
 for(const [key,row] of attendanceGroups){
  ...
  else daily.set(dayKey,{branchId,employeeId,employee:u.name,date,distanceMeters:distance,rate:u.travelRatePerKm??company?.travelRatePerKm??null});
 }
 ...
 for(const r of [...daily.values()].filter(x=>x.rate)){
  const amount=new Prisma.Decimal(r.distanceMeters).div(1000).mul(r.rate!).toDecimalPlaces(2),key=`${r.branchId}:${r.employeeId}:${r.date}`,existing=beforeMap.get(key);
  if(!existing)mutations.push(db.dailyTravelApproval.create({data:{companyId:actor.companyId,branchId:r.branchId,employeeId:r.employeeId,businessDate:dbDate(r.date),distanceMeters:r.distanceMeters,ratePerKm:r.rate!,amount,status:"PENDING"}}));
 ...
 const rows=[...daily.values()].sort((a,b)=>b.date.localeCompare(a.date)||a.employee.localeCompare(b.employee)).map(r=>{
  const approval=approvalMap.get(`${r.branchId}:${r.employeeId}:${r.date}`);
  return{key:`${r.branchId}:${r.employeeId}:${r.date}`,...r,ratePerKm:approval?.ratePerKm??r.rate,amount:approval?.amount??(r.rate?new Prisma.Decimal(r.distanceMeters).div(1000).mul(r.rate).toDecimalPlaces(2):null),status:approval?.status??null,reviewedAt:approval?.reviewedAt??null};
 });
