...
export async function gpsReport(raw:SearchParams){
 const actor=await reportActor(),filters=parseReportFilters(raw),ids=await resolveEmployeeScope(actor,filters.employeeId),attendanceId=typeof raw.attendanceId==="string"?raw.attendanceId:undefined;
