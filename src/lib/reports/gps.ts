...
export async function gpsReport(raw:SearchParams,providedActor?:ReportActor){
 ...
 for(const s of sessions){
  ...
  for(const v of s.customerVisits){if(v.checkedInAt>=start&&v.checkedInAt<end){events.push({id:`${v.id}:in`,type:"CHECK_IN",at:v.checkedInAt,sessionId:s.id,customer:v.customer?.name||v.contactName||"Field prospect",detail:v.customer?.address||v.visitNotes||undefined,latitude:v.checkInLatitude,longitude:v.checkInLongitude});markers.push({latitude:v.checkInLatitude,longitude:v.checkInLongitude,label:`Check-in: ${v.customer?.name||v.contactName||"Field prospect"}`})}if(v.checkedOutAt&&v.checkedOutAt>=start&&v.checkedOutAt<end){events.push({id:`${v.id}:out`,type:"CHECK_OUT",at:v.checkedOutAt,sessionId:s.id,customer:v.customer?.name||v.contactName||"Field prospect",detail:v.checkoutSentiment||v.checkoutRemarks||undefined,latitude:v.checkOutLatitude??undefined,longitude:v.checkOutLongitude??undefined});if(v.checkOutLatitude!=null&&v.checkOutLongitude!=null)markers.push({latitude:v.checkOutLatitude,longitude:v.checkOutLongitude,label:`Check-out: ${v.customer?.name||v.contactName||"Field prospect"}`})}}
