...
export async function checkInReport(raw:SearchParams, advanced=false,providedActor?:ReportActor,exportMode=false) {
  ...
  const rows=await Promise.all(visits.map(async v=>{
    const prior=advanced?await db.customerVisit.findMany({where:{companyId:actor.companyId,branchId:v.branchId,customerId:v.customerId,OR:[{checkedInAt:{lt:v.checkedInAt}},{checkedInAt:v.checkedInAt,id:{lt:v.id}}]},select:{id:true,checkedInAt:true},take:1}):[];
