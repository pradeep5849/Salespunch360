...
export function validateRetentionSelection(currentUsage:number,limit:number,ids:string[],allowReduction:boolean){
 ...
 // An empty selection intentionally means "use the automatic oldest-active retention fallback".
 if(ids.length===0)return;
 if(ids.length!==limit||new Set(ids).size!==ids.length)throw new Error('SEAT_SELECTION_REQUIRED');
}
