...
export async function applyDueSeatReductions(companyId:string,now=new Date()){
 await db.$transaction(async tx=>{
  ...
  for(const item of byRole){const users=active.filter(u=>u.salesRole===item.role);if(users.length>item.limit){const retained=item.retain.length===item.limit?new Set(item.retain):new Set(users.slice(0,item.limit).map(u=>u.id));for(const u of users)if(!retained.has(u.id))suspendedUserIds.push(u.id)}}
