...
export async function applyDueSeatReductions(companyId:string,now=new Date()){
 await db.$transaction(async tx=>{
  ...
  const active=await tx.user.findMany({where:{companyId,isActive:true,salesAccessActive:true,salesRole:{in:["ADMIN","MANAGER","SALES"]}},select:{id:true,salesRole:true,createdAt:true},orderBy:[{createdAt:"asc"},{id:"asc"}]});
  ...
  for(const item of byRole){const users=active.filter(u=>u.salesRole===item.role);if(users.length>item.limit){const validRetain=item.retain.filter(id=>users.some(u=>u.id===id));const retained=validRetain.length===item.limit?new Set(validRetain):new Set(users.slice(0,item.limit).map(u=>u.id));for(const u of users)if(!retained.has(u.id))suspendedUserIds.push(u.id)}}
  ...
  await tx.billingAuditEvent.create({data:{companyId,type:"SEATS_CHANGED",entityId:sub.id,metadata:{reason:"PAID_RENEWAL_SALES_SEAT_REDUCTION_LAST_ADDED_FIRST",adminSeats:order.adminSeats,managerSeats:order.managerSeats,salesSeats:order.salesSeats,suspendedSalesUserIds:suspendedUserIds}}});
 });
}
