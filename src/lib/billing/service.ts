...
const admin=async()=>{const u=await requireRole("COMPANY_ADMIN");if(!u.companyId)throw new Error("NOT_AUTHORIZED");return{...u,companyId:u.companyId}};
...
async function validatedRetainedUsers(companyId:string,role:"MANAGER"|"SALES",newSeats:number,currentUsage:number,ids:string[],isRenewal:boolean){
 ...
 const count=await db.user.count({where:{companyId,role,isActive:true,id:{in:ids}}});
 if(count!==ids.length)throw new Error("INVALID_SEAT_SELECTION");
 return ids;
}

export async function createBillingOrder(raw:unknown){
 const u=await admin(),d=orderRequestSchema.parse(raw);
 ...
 const prices=await currentPrices(),mp=prices.find(p=>p.role==="MANAGER"&&p.period===d.billingPeriod),sp=prices.find(p=>p.role==="SALES"&&p.period===d.billingPeriod);
 if(!mp||!sp||mp.currency!==sp.currency)throw new Error("PRICING_UNAVAILABLE");
 const money=calculateOrder(d.managerSeats,d.salesSeats,mp.amount,sp.amount);
 return db.$transaction(async tx=>{
  const o=await tx.billingOrder.create({data:{companyId:u.companyId,createdByUserId:u.id,billingPeriod:d.billingPeriod,managerSeats:d.managerSeats,salesSeats:d.salesSeats,retainManagerUserIds,retainSalesUserIds,managerUnitPrice:mp.amount,salesUnitPrice:sp.amount,currency:"INR",...money,provider:"UNCONFIGURED",idempotencyKey:d.idempotencyKey,expiresAt:new Date(Date.now()+86400000)}});
  await tx.billingAuditEvent.create({data:{companyId:u.companyId,actorUserId:u.id,type:"ORDER_CREATED",entityId:o.id,metadata:{period:d.billingPeriod,managerSeats:d.managerSeats,salesSeats:d.salesSeats,retainManagerUserIds,retainSalesUserIds}}});
  return o;
 });
}
...
export async function activateVerifiedPayment(v:VerifiedPayment){
 const result=await db.$transaction(async tx=>{
  ...
  const sub=await tx.companySubscription.create({data:{companyId:order.companyId,status:"ACTIVE",billingPeriod:order.billingPeriod,managerSeats:order.managerSeats,salesSeats:order.salesSeats,startsAt:start,endsAt:end,sourceOrderId:order.id}});
...
export async function billingDashboard(){
 ...
 const [company,prices,orders,activeEmployees]=await Promise.all([
  ...
  db.user.findMany({where:{companyId:u.companyId,isActive:true,role:{in:["MANAGER","SALES"]}},select:{id:true,name:true,role:true,managerType:true},orderBy:[{role:"asc"},{name:"asc"}]})
 ]);
...
export async function manualOverride(raw:unknown){
 ...
 return db.$transaction(async tx=>{
  const usage=await tx.user.groupBy({by:["role"],where:{companyId:d.companyId,isActive:true,role:{in:["MANAGER","SALES"]}},_count:true});
  const m=usage.find(x=>x.role==="MANAGER")?._count??0,s=usage.find(x=>x.role==="SALES")?._count??0;
  if(d.managerSeats<m||d.salesSeats<s)throw new Error("SEATS_BELOW_USAGE");
  const sub=await tx.companySubscription.create({data:{companyId:d.companyId,status:d.status,billingPeriod:d.billingPeriod,managerSeats:d.managerSeats,salesSeats:d.salesSeats,startsAt:now,endsAt:end}});
