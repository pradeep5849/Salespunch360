...
export async function assignCustomer(raw:unknown){const admin=await requireRole("COMPANY_ADMIN");if(!admin.companyId)throw new Error("NOT_AUTHORIZED");const companyId=admin.companyId;const d=assignCustomerSchema.parse(raw);return db.$transaction(async tx=>{const assignee=await tx.user.findFirst({where:{id:d.assignedUserId,companyId,isActive:true,OR:[{role:"SALES"},{role:"MANAGER",managerType:"FIELD_MANAGER"}]},select:{id:true}});if(!assignee)throw new Error("INVALID_ASSIGNMENT");const changed=await tx.customer.updateMany({where:{id:d.customerId,companyId},data:{assignedUserId:assignee.id}});if(changed.count!==1)throw new Error("CUSTOMER_NOT_FOUND");});}
