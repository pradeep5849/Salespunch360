...
export async function dashboardData(raw:{checkInEmployee?:string;liveEmployee?:string}){
 ...
 const [company,openAttendance,visits,presentCount,todayVisitCount,todayLeadCount,latestLocation,targets]=await Promise.all([
  ...
  db.customerVisit.findMany({where:{companyId:actor.companyId,userId:checkUserId?checkUserId:{in:teamIds}},orderBy:{checkedInAt:"desc"},take:8,select:{id:true,checkedInAt:true,checkedOutAt:true,checkInLatitude:true,checkInLongitude:true,checkoutSentiment:true,checkoutRemarks:true,visitNotes:true,user:{select:{name:true}},customer:{select:{name:true,address:true}}}}),
