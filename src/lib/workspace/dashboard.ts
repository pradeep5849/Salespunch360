...
export async function dashboardData(raw:{checkInEmployee?:string;liveEmployee?:string}){
 ...
 const employees=await db.user.findMany({where:dashboardEmployeeWhere(actor),select:{id:true,name:true,role:true,salesRole:true,managerType:true},orderBy:{name:"asc"}});
 const checkInEmployees=employees.filter(e=>e.salesRole==="SALES"||(e.salesRole==="MANAGER"&&e.managerType!=="MANAGER_ONLY"));
 const allowed=new Set(employees.map(e=>e.id)),checkInAllowed=new Set(checkInEmployees.map(e=>e.id));
 const requestedCheck=raw.checkInEmployee,checkUserId=isSales(actor)?actor.id:requestedCheck&&checkInAllowed.has(requestedCheck)?requestedCheck:undefined;
 ...
 const employeeIds=employees.map(e=>e.id),checkInEmployeeIds=checkInEmployees.map(e=>e.id);
 const teamIds=isSales(actor)?[actor.id]:isManager(actor)?(fieldManager?[actor.id,...employeeIds]:employeeIds):employeeIds;
 const checkInTeamIds=isSales(actor)?[actor.id]:isManager(actor)?(fieldManager?[actor.id,...checkInEmployeeIds]:checkInEmployeeIds):checkInEmployeeIds;
 ...
 const [company,openAttendance,visits,presentCount,todayVisitCount,todayLeadCount,monthVisitCount,monthLeadCount,latestLocation,pendingTodayTasks,overdueTasks]=await Promise.all([
  ...
  db.customerVisit.count({where:{companyId:actor.companyId,branchId:branches.branchId,userId:{in:checkInTeamIds},checkedInAt:{gte:indiaToday,lt:indiaTomorrow}}}),
  ...
  db.customerVisit.count({where:{companyId:actor.companyId,branchId:branches.branchId,userId:{in:checkInTeamIds},checkedInAt:{gte:indiaMonthStart,lt:indiaTomorrow}}}),
