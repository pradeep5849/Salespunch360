...
export async function dashboardData(raw:{checkInEmployee?:string;liveEmployee?:string}){
 ...
 const [company,openAttendance,visits,presentCount,todayVisitCount,todayLeadCount,monthVisitCount,monthLeadCount,latestLocation,pendingTodayTasks,overdueTasks]=await Promise.all([
  ...
  (isAdmin(actor)||(isManager(actor)&&!fieldManager))?null:db.attendance.findFirst({where:{companyId:actor.companyId,userId:actor.id,endedAt:null},include:{_count:{select:{locationPoints:true}}}}),
  ...
  (isAdmin(actor)||(isManager(actor)&&!fieldManager))?0:db.followUpTask.count({where:{companyId:actor.companyId,assignedUserId:actor.id,status:"PENDING",dueDate:{gte:indiaToday,lt:indiaTomorrow}}}),
  (isAdmin(actor)||(isManager(actor)&&!fieldManager))?0:db.followUpTask.count({where:{companyId:actor.companyId,assignedUserId:actor.id,status:"PENDING",dueDate:{lt:indiaToday}}})
 ]);
