...
export async function mobileFieldContext(user:MobilePrincipal,query=''){const principal=field(user);const [customers,visits]=await Promise.all([searchCustomersForCompany(principal.companyId,query),getOwnVisitHistoryForUser(principal)]);return{customers:customers.map(({id,name,contactPerson,phone,email,address,latitude,longitude})=>({id,name,contactPerson,phone,email,address,latitude,longitude})),visits:visits.map(({_count,...visit})=>({...visit,leadCount:_count.leads}))}}
