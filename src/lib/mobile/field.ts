...
export async function mobileFieldContext(user:MobilePrincipal,query=''){const principal=field(user),branches=await operationalBranchContext(principal);const [customers,visits]=await Promise.all([searchCustomersForCompany(principal.companyId,query,principal.id,branches.branchIds),getOwnVisitHistoryForUser(principal)]);return{branches:branches.branches.map(({id})=>({id})),customers:customers.map(({id,name,contactPerson,phone,email,address,latitude,longitude,checkInReferenceSetAt})=>({id,name,contactPerson,phone,email,address,latitude,longitude,checkInReferenceSetAt})),visits:visits.map(({_count,...visit})=>({...visit,leadCount:_count.sourceLeads}))}}
