...
export type MobileAppPrincipal = {
  id: string;
  name: string;
  email: string;
  companyId: string;
  ...
  authorizedWorkspaces: WebWorkspace[];
};
export type MobileSalesPrincipal = MobileAppPrincipal & { salesRole: SalesRole };
/** Stable native Sales service principal; app authentication is narrowed before constructing it. */
...
export function isMobileEligible(user: Eligibility, edition: ProductEdition | null): user is Eligibility & { companyId: string } {
  return edition !== null && resolveWorkspaceAccess({...user,accountRole:user.accountRole??null,accountAccessActive:user.accountAccessActive??false,managerType:user.managerType??null},edition).defaultWorkspace !== null;
}
...
export async function createMobileSession(identifier: string, password: string, now = new Date()) {
  const user = await db.user.findFirst({
    ...
    select: { id:true,name:true,email:true,passwordHash:true,role:true,salesRole:true,accountRole:true,salesAccessActive:true,accountAccessActive:true,managerType:true,companyId:true,isActive:true,company:{select:{productEdition:true}} },
  });
  ...
  const lockedUser=await db.$transaction(async tx=>{
    ...
    return {...current,productEdition:company!.productEdition};
  });
...
export async function authenticateMobileToken(authorization:string|null,now=new Date()):Promise<MobileAppPrincipal>{
  ...
  const productEdition=session.user.company!.productEdition,access=resolveWorkspaceAccess(session.user,productEdition),authorizedWorkspaces=([access.canAccessSales&&"SALES",access.canAccessAccount&&"ACCOUNT"].filter(Boolean) as WebWorkspace[]);
