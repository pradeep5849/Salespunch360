...
async function mobileSalesDashboard(user:MobileAppPrincipal|MobilePrincipal,branchId:Prisma.StringFilter|string|null){
 ...
 const [todayVisitCount,todayLeadCount,monthVisitCount,monthLeadCount,pendingTodayTasks,overdueTasks,recentVisits]=await Promise.all([
  ...
  db.customerVisit.findMany({where:{...scope,userId:user.id,checkedOutAt:{not:null}},orderBy:[{checkedOutAt:"desc"},{id:"desc"}],take:8,select:{id:true,contactName:true,checkedInAt:true,checkedOutAt:true,checkInAddress:true,checkoutSentiment:true,photo:{select:{id:true}},customer:{select:{name:true}}}})
 ]);
 return{todayVisitCount,todayLeadCount,monthVisitCount,monthLeadCount,pendingTodayTasks,overdueTasks,recentVisits:recentVisits.map(v=>({id:v.id,contactName:v.contactName,checkedInAt:v.checkedInAt,checkedOutAt:v.checkedOutAt,checkInAddress:v.checkInAddress,checkoutSentiment:v.checkoutSentiment,customerName:v.customer?.name??null,thumbnailUrl:v.photo?`api/v1/mobile/visit-photos/${v.id}/thumbnail`:null}))};
}
...
export async function mobileBootstrap(user:MobileAppPrincipal|MobilePrincipal){
 ...
 const [company,attendance,entitlement,salesDashboard]=await Promise.all([db.company.findUnique({where:{id:user.companyId},select:{name:true,logoObjectKey:true,addressLine1:true,addressLine2:true,locality:true,city:true,state:true,postalCode:true,country:true,teamStructure:true,attendanceEnabled:true,gpsTrackingEnabled:true}}),mobileFieldWorkEnabled(user)?db.attendance.findFirst({where:{companyId:user.companyId,branchId:branches.branchId!,userId:user.id,endedAt:null},select:{id:true,startedAt:true,_count:{select:{locationPoints:true}}}}):Promise.resolve(null),effectiveEntitlement(user.companyId),sales?mobileSalesDashboard(user,branches.branchId):Promise.resolve(null)]);
 ...
 const activeAttendance=attendance?{id:attendance.id,startedAt:attendance.startedAt,gpsPointCount:attendance._count.locationPoints}:null;
 return{productEdition,authorizedWorkspaces,canSwitchWorkspace:authorizedWorkspaces.length===2,branches:branches.branches.map(({id})=>({id})),branchSelectionRequired:sales&&branches.branchIds.length>1,user:{id:user.id,name:user.name,email:user.email,salesRole:user.salesRole,accountRole,managerType:user.managerType},company:{name:company.name,logoUrl:company.logoObjectKey?"api/v1/mobile/company-logo":null,address:address||null},teamStructure:company.teamStructure,features:{attendanceEnabled:sales&&company.attendanceEnabled,gpsTrackingEnabled:sales&&company.gpsTrackingEnabled,fieldWorkEnabled:mobileFieldWorkEnabled(user)},capabilities:{canManageEmployees:mobileCan(user,"SALES_USER_ADMIN"),canManageSalesSettings:mobileCan(user,"SALES_SETTINGS"),canAccessSalesBilling:mobileCan(user,"SALES_BILLING"),canViewReports:mobileCan(user,"SALES_REPORTS")},entitlement:{state:entitlement.state,operationalWritesAllowed:entitlement.operationalWritesAllowed,adminLimit:entitlement.adminLimit,adminUsage:entitlement.adminUsage,managerLimit:entitlement.managerLimit,salesLimit:entitlement.salesLimit,managerUsage:entitlement.managerUsage,salesUsage:entitlement.salesUsage},attendance:activeAttendance,salesDashboard};
}