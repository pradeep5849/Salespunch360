...
import {attachTelecallerToBillingOrder,quoteUnifiedTelecaller,type TeamPurchaseMode} from "@/lib/billing/unified-team";
...
export async function mobileBillingQuote(p:MobilePrincipal,raw:unknown){
 ...
 const accountUnit=plus?(prices.find(x=>x.role==="ACCOUNT_PACKAGE"&&x.period===period)?.amount??new Prisma.Decimal(period==="SIX_MONTH"?ACCOUNT_PACKAGE_SIX_MONTH_PRICE_INR:ACCOUNT_PACKAGE_YEARLY_PRICE_INR)):new Prisma.Decimal(0),now=new Date(),active=await db.companySubscription.findMany({where:{companyId:p.companyId,status:"ACTIVE",startsAt:{lte:now},endsAt:{gt:now}},select:{adminSeats:true,managerSeats:true,salesSeats:true,accountPackages:true,startsAt:true,endsAt:true,sourceOrder:{select:{provider:true}}},orderBy:{endsAt:"desc"}}),currentTerm=effectiveCurrentTerm(active),entitlement=await effectiveEntitlement(p.companyId,now),tele=await getTelecallerBillingOverviewForCompany(p.companyId),mode:TeamPurchaseMode=input.kind==="ADD_TEAM"||Boolean(entitlement.paidActive&&currentTerm&&period===entitlement.subscription?.billingPeriod&&(a>entitlement.adminLimit||m>entitlement.managerLimit||s>entitlement.salesLimit))?"ADD_TEAM":entitlement.paidActive?"RENEW":"NEW",requestedTelecaller=Number(input.quantity),targetTelecaller=mode==="ADD_TEAM"&&input.kind==="ADD_TEAM"&&Number.isInteger(requestedTelecaller)?tele.limit+Math.max(0,requestedTelecaller):Number.isInteger(requestedTelecaller)?Math.max(0,requestedTelecaller):tele.limit,quote=quoteCombinedOrder({period:period as "SIX_MONTH"|"YEARLY",adminSeats:a,managerSeats:m,salesSeats:s,accountPackages:plus?accountPackages:0,adminPrice:ap.amount,managerPrice:mp.amount,salesPrice:sp.amount,accountPrice:plus?accountUnit:undefined,now,currentTerm}),teleQuote=await quoteUnifiedTelecaller(p.companyId,period as "SIX_MONTH"|"YEARLY",targetTelecaller,mode,now);
 const ids=(key:string)=>Array.isArray(input[key])?input[key].filter((x):x is string=>typeof x==="string"):[];
 return{kind:mode==="ADD_TEAM"?"ADD_TEAM":plus?"PLUS":"SALES",billingPeriod:period,adminSeats:a,managerSeats:m,salesSeats:s,accountPackages,quantity:targetTelecaller,retainAdminUserIds:ids("retainAdminUserIds"),retainManagerUserIds:ids("retainManagerUserIds"),retainSalesUserIds:ids("retainSalesUserIds"),totalAmount:quote.subtotal.plus(teleQuote.subtotal).toFixed(2),salesSubtotal:quote.salesSubtotal.plus(teleQuote.subtotal).toFixed(2),accountSubtotal:quote.accountSubtotal.toFixed(2),prorated:quote.prorated||teleQuote.prorated,coTermEndsAt:quote.coTermEndsAt??teleQuote.coTermEndsAt,currency:"INR"};
}
