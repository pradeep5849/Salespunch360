...
const user:MobilePrincipal={id:'user-1',name:'Ada',email:'ada@example.com',role:'COMPANY_ADMIN',managerType:null,companyId:'company-1'};
...
describe('mobile boundary',()=>{
 ...
 it('rejects role and tenant mass assignment',()=>expect(mobileLoginSchema.safeParse({identifier:'user@example.com',password:'long-enough',role:'COMPANY_ADMIN',managerType:null,companyId:'x'}).success).toBe(false));
 it('enforces strong matching mobile password changes without tenant fields',()=>{expect(mobilePasswordSchema.safeParse({currentPassword:'old',newPassword:'StrongPassword1',confirmPassword:'StrongPassword1'}).success).toBe(true);expect(mobilePasswordSchema.safeParse({currentPassword:'old',newPassword:'weak-password',confirmPassword:'weak-password'}).success).toBe(false);expect(mobilePasswordSchema.safeParse({currentPassword:'old',newPassword:'StrongPassword1',confirmPassword:'StrongPassword1',companyId:'other'}).success).toBe(false)});
