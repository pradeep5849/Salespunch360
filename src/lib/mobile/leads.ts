...
export async function mobileLead(u:MobilePrincipal,id:string){const leadId=z.string().uuid().parse(id),lead=await getLeadForActor(actor(u),leadId);if(!lead)throw new Error('NOT_FOUND');if(lead.stage==='WON')await ensureWonLeadProjectForActor({id:u.id,companyId:u.companyId},lead.id);const normalized={...lead,visits:lead.visits.map(v=>({id:v.id,userName:v.user.name,checkedInAt:v.checkedInAt,checkedOutAt:v.checkedOutAt,checkInAddress:v.checkInAddress,visitNotes:v.visitNotes})),activities:lead.activities.map(a=>({...a,actorUser:{name:a.actorUser.name}}))};return withVisitCount(normalized,u)}
