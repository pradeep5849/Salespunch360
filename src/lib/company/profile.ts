...
export const companyProfileSchema = z.object({
  ...
  alternatePhone: optional(30), website: z.union([z.literal(""), z.string().trim().url().max(300)]).transform(v => v || null), gstin: optionalBusinessId(/^\d{2}[A-Z]{5}\d{4}[A-Z][1-9A-Z]Z[0-9A-Z]$/, "Enter a valid GSTIN"), pan: optionalBusinessId(/^[A-Z]{5}\d{4}[A-Z]$/, "Enter a valid PAN"), registrationNumber: optional(60), description: optional(2000),
  teamStructure: z.enum(["MANAGERS_AND_SALES", "SALES_ONLY"]),
  enabledModules: z.array(z.enum(SELECTABLE_SALES_MODULES)).max(SELECTABLE_SALES_MODULES.length),
}).strict();
