...
describe("pending conversion regression contracts",()=>{
 ...
 it("keeps Pending a pseudo-view instead of a LeadStage",()=>{expect(page).toContain('q.view==="pending"');expect(page).toContain('href={scoped({view:"pending"})}');expect(page).not.toContain('stage:"PENDING"')});
});
