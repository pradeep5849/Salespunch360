...
export function pendingVisitWhere(actor:PipelineActor){return {companyId:actor.companyId,visitType:"NEW" as const,leadId:null,OR:[{contactPhone:null},{contactPhone:""}],...(actor.role==="SALES"?{userId:actor.id}:actor.role==="MANAGER"?{AND:[{OR:[{userId:actor.id},{user:{role:"SALES" as const,managerId:actor.id}}]}]}:{})};}
