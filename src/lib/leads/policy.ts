...
export function transitionKind(role:Role,from:LeadStage,to:LeadStage){if(from===to)throw new LeadError("TERMINAL");if(activeStages.includes(from))return to==="WON"?"WON":to==="LOST"?"LOST":"STAGE_CHANGED";if(role!=="COMPANY_ADMIN"||!activeStages.includes(to))throw new LeadError("TERMINAL");return "REOPENED";}
