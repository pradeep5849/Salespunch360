...
describe("lead creation restrictions",()=>{
  it("rejects standalone lead creation by SALES before any database work",async()=>{
    await expect(createLeadForActor({id:"sales",name:"Sales",email:"s@example.com",salesRole:"SALES",managerType:null,companyId:"company"},{title:"Bypass",assignedUserId:"sales"})).rejects.toThrow("NOT_FOUND");
  });
});
