...
describe("nullable customer visit tenant trigger repair",()=>{
 ...
 it("keeps standalone NEW visits without a customer assignment",()=>{const newPath=service.slice(service.indexOf('}else{name=d.name'),service.indexOf('setStage("CREATE_VISIT")'));expect(newPath).not.toContain("customerId=");expect(service).toContain("data:{id:visitId,companyId:user.companyId,branchId,userId:user.id,customerId,leadId,attendanceId");});
});
