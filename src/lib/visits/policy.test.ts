...
describe("visit workflow policy",()=>{
 ...
 it("blocks pending visits only when checkout is required",()=>{expect(()=>assertPendingVisitAllowed(true,1)).toThrow("CHECKOUT_REQUIRED");expect(()=>assertPendingVisitAllowed(false,2)).not.toThrow()});
