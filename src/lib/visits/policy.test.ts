...
describe("visit workflow policy",()=>{
 ...
 it("restricts Manager visibility to self and assigned canonical Sales",()=>{expect(canManagerViewVisit("m1",{id:"m1",salesRole:"MANAGER",managerId:null})).toBe(true);expect(canManagerViewVisit("m1",{id:"s1",salesRole:"SALES",managerId:"m1"})).toBe(true);expect(canManagerViewVisit("m1",{id:"s2",salesRole:"SALES",managerId:"m2"})).toBe(false);expect(canManagerViewVisit("m1",{id:"legacy-only",salesRole:null,managerId:"m1"})).toBe(false)});
