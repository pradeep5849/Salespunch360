...
const photo = z.string().startsWith("data:image/").max(7_000_000);
export const fieldCheckInSchema=z.discriminatedUnion("visitType",[
 z.object({visitType:z.literal("NEW"),name:z.string().trim().min(2).max(160),phone,photoDataUrl:photo,location:requiredGps,visitNotes:optionalNotes}).strict(),
 z.object({visitType:z.literal("FOLLOW_UP"),leadId:z.string().uuid(),photoDataUrl:photo.optional(),location:requiredGps,visitNotes:optionalNotes}).strict(),
 z.object({visitType:z.literal("CUSTOMER"),customerId:z.string().uuid(),photoDataUrl:photo.optional(),location:requiredGps,visitNotes:optionalNotes}).strict(),
]);
