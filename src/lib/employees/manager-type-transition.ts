...
export async function assertManagerOnlyTransitionSafe(tx: Prisma.TransactionClient, companyId: string, userId: string) {
  const [attendance, visit, lead, task, customer, target, assignedUser] = await Promise.all([
    ...
    tx.user.findFirst({where:{companyId,managerId:userId,salesRole:"SALES"},select:{id:true}}),
  ]);
  if (attendance || visit || lead || task || customer || target || assignedUser) throw new Error("MANAGER_TYPE_CONFLICT");
}
