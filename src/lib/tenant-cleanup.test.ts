...
describe("database safety contracts", () => {
  ...
  it("implements liveness with SELECT 1 on the locked transaction client", () => { expect(source).toContain('runCleanupDatabaseStage("ASSERT_TRANSACTION_ALIVE", () => tx.$queryRaw`SELECT 1`)'); expect(source).not.toContain('db.$queryRaw`SELECT 1`'); });
  ...
  it("projects an integer while forcing the transaction advisory lock to execute", () => { expect(source).toContain('WITH "company_lock" AS MATERIALIZED (SELECT pg_advisory_xact_lock'); expect(source).toContain('SELECT 1::int AS "locked" FROM "company_lock"'); expect(source).not.toMatch(/\$queryRaw[^`]*`SELECT pg_advisory_xact_lock/); });
  it("uses the transaction client and locked Company id for authoritative work", () => { expect(source).toContain("work(lockedAdapter(tx, company[0].id))"); expect(source).not.toContain("work(lockedAdapter(db"); });
