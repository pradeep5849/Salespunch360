...
class FoundationTest{
 ...
 @Test fun `sales report drawer matches web mobile links`(){assertEquals(listOf("Check-in Report","My Attendance"),salesReportDrawerItems(MobileRole.SALES).map{it.label})}
 @Test fun `admin report drawer matches web mobile links`(){assertEquals(listOf("Check-in Report","Attendance Report","GPS Report","Geofence Report","Target Analysis","Expense Report"),salesReportDrawerItems(MobileRole.PRIMARY_ADMIN).map{it.label})}
