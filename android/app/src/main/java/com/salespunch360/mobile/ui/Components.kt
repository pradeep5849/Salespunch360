...
@Composable fun CompanyIdentity(name:String,address:String?=null){Row(verticalAlignment=Alignment.CenterVertically){Surface(shape=MaterialTheme.shapes.medium,color=MaterialTheme.colorScheme.primaryContainer){Box(Modifier.size(40.dp),contentAlignment=Alignment.Center){Text(initials(name),fontWeight=FontWeight.Bold)}};Spacer(Modifier.width(10.dp));Column{Text(name,fontWeight=FontWeight.SemiBold,maxLines=1);address?.let{Text(it,style=MaterialTheme.typography.labelSmall,maxLines=1)}}}}
