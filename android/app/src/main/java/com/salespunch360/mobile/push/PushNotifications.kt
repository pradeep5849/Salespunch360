...
object PushNotifications {
 ...
 fun show(context:Context,title:String?,body:String?){if(body.isNullOrBlank())return;val manager=context.getSystemService(NotificationManager::class.java);if(Build.VERSION.SDK_INT>=26)manager.createNotificationChannel(NotificationChannel(CHANNEL_ID,"Field activity",NotificationManager.IMPORTANCE_DEFAULT));if(Build.VERSION.SDK_INT<33||ContextCompat.checkSelfPermission(context,Manifest.permission.POST_NOTIFICATIONS)==PackageManager.PERMISSION_GRANTED)NotificationManagerCompat.from(context).notify((System.currentTimeMillis()%Int.MAX_VALUE).toInt(),NotificationCompat.Builder(context,CHANNEL_ID).setSmallIcon(R.drawable.ic_sp360_badge).setContentTitle(title?:"SalesPunch360 field activity").setContentText(body).setStyle(NotificationCompat.BigTextStyle().bigText(body)).setAutoCancel(true).build())}
}
