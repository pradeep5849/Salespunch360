...
@Composable
fun AdminManagerAuthenticatedApp(data: Bootstrap, message: String?, dismiss: () -> Unit, attendance: (Boolean, LocationPayload, () -> Unit) -> Unit, logout: () -> Unit, switchToAccount: (() -> Unit)? = null) {
    val role = data.user.salesRole ?: return
    val drawer = rememberDrawerState(DrawerValue.Closed)
    val scope = rememberCoroutineScope()
    var route by rememberSaveable(role) { mutableStateOf("Dashboard") }
    ...
    fun navigate(value: String) { route = value; reportType = null; visitTask = null; scope.launch { drawer.close() } }

    BackHandler(enabled = visitTask != null) { visitTask = null }
    BackHandler(enabled = visitTask == null && (route != "Dashboard" || reportType != null)) {
        pendingLeadId = null
        reportType = null
        reportOpen = false
        route = "Dashboard"
    ...
    ModalNavigationDrawer(
        ...
        Scaffold(containerColor = SalesPale, topBar = {
            TopAppBar(colors = TopAppBarDefaults.topAppBarColors(containerColor = Color.White), navigationIcon = { IconButton(onClick = { scope.launch { drawer.open() } }) { Icon(Icons.Default.Menu, "Open menu") } }, title = { CompanyIdentity(data.company.name, data.company.address, data.company.logoUrl) }, actions = {
                Box {
            ...
            Column(Modifier.padding(padding).fillMaxSize()) {
                ...
                when {
                    visitTask != null -> RoleFollowUpVisit(visitTask!!) { visitTask = null }
                    route == "Dashboard" -> RoleDashboard(data)
                    route == "Employees" -> EmployeesScreen()
                    route == "Attendance" -> if (role == MobileRole.MANAGER && data.user.managerType != "MANAGER_ONLY") FieldManagerAttendanceScreen(data, attendance) else TeamAttendanceScreen(role)
                    ...
                    route == "Billing & Subscription" -> if (role != MobileRole.ADMIN) SubscriptionScreen() else RoleDashboard(data)
