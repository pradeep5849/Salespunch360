...
import com.salespunch360.mobile.ui.EmployeesScreen
...
private fun moneyMode(path:String)=when{path.startsWith("/workspace/account/money/accounts")->"accounts";path.startsWith("/workspace/account/money/transfers")->"transfers";path.startsWith("/workspace/account/money/capital")->"owners";path.startsWith("/workspace/account/money/loans")->"loans";else->null}

private fun accountingMode(path:String)=when{path.startsWith("/workspace/account/accounting/accounts")->"ledgers";path.startsWith("/workspace/account/accounting/journals")->"journals";path.startsWith("/workspace/account/utilities/financial-year")->"financial-years";path.startsWith("/workspace/account/utilities/period-locks")->"period-locks";else->null}
...
private fun adminMode(path:String)=when{path=="/workspace/account/notifications"->"notifications";path=="/workspace/account/settings"->"settings";path.endsWith("/settings/custom-fields")->"custom-fields";path.endsWith("/settings/modules")->"modules";path.endsWith("/settings/print-templates")->"print-templates";path=="/workspace/account/tax"||path.endsWith("/tax/reports")->"tax-reports";path.endsWith("/tax/settings")->"tax-settings";path.endsWith("/utilities/audit")->"audit";path.endsWith("/utilities/recycle-bin")->"recycle-bin";path.endsWith("/utilities/verification")->"verification";path.endsWith("/utilities/import")->"imports";else->null}

private fun utilityMode(path:String)=when{path.endsWith("/utilities/import")||path.contains("/utilities/import/")->"import";path.endsWith("/utilities/export")->"export";path.endsWith("/utilities/backup")->"backup";path.endsWith("/utilities/recycle-bin")->"recycle";else->null}
