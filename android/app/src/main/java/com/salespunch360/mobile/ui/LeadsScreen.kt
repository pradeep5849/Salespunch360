...
@Composable fun LeadsScreen(initialLeadId:String?=null,onInitialLeadConsumed:()->Unit={},onCheckIn:(LeadSummary)->Unit={},vm:LeadsViewModel=viewModel()){
 val state=vm.state.collectAsStateWithLifecycle().value;var search by remember{mutableStateOf("")};var stage by remember{mutableStateOf<LeadStage?>(null)};var followLead by remember{mutableStateOf<LeadSummary?>(null)};var transitionLead by remember{mutableStateOf<LeadSummary?>(null)}
 ...
 LazyColumn(Modifier.fillMaxSize().padding(horizontal=16.dp),contentPadding=PaddingValues(vertical=14.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
  item{Text("Leads",style=MaterialTheme.typography.headlineSmall,fontWeight=FontWeight.Bold,color=SalesInk);Text("Manage your pipeline and next actions.",color=SalesMuted);Spacer(Modifier.height(8.dp));OutlinedTextField(search,{search=it},Modifier.fillMaxWidth(),label={Text("Search leads")},singleLine=true,leadingIcon={Icon(Icons.Default.Search,null)});Spacer(Modifier.height(8.dp));LazyRow(horizontalArrangement=Arrangement.spacedBy(6.dp)){item{FilterChip(stage==null,{stage=null},{Text("All ${state.leads.size}")})};items(LeadStage.entries){s->val count=state.leads.count{it.stage==s};FilterChip(stage==s,{stage=s},{Text("${stageLabel(s)} $count")})}};state.message?.let{MessageBanner(it,vm::clear)}}
  if(visible.isEmpty())item{ContentCard("No leads found","Try another search or pipeline filter.")}
  items(visible,key={it.id}){lead->LeadCard(lead,state.busy,onFollowUp={followLead=lead},onCheckIn={onCheckIn(lead)},onTransition={transitionLead=lead})}
 }
...
@Composable private fun LeadCard(lead:LeadSummary,busy:Boolean,onFollowUp:()->Unit,onCheckIn:()->Unit,onTransition:()->Unit){OutlinedCard(Modifier.fillMaxWidth()){Column(Modifier.padding(16.dp),verticalArrangement=Arrangement.spacedBy(7.dp)){Row{Text(lead.title,Modifier.weight(1f),style=MaterialTheme.typography.titleMedium,fontWeight=FontWeight.Bold,maxLines=1,overflow=TextOverflow.Ellipsis);StatusChip(stageLabel(lead.stage))};Text(lead.customer?.name?:lead.companyName?:lead.contactName?:"Prospect",color=SalesInk);lead.phone?.let{Text(it,style=MaterialTheme.typography.bodySmall,color=SalesMuted)};Text("Assigned to ${lead.assignedUser.name}",style=MaterialTheme.typography.bodySmall,color=SalesMuted);Text("Visits ${lead.visitCount}",style=MaterialTheme.typography.bodySmall,fontWeight=FontWeight.SemiBold,color=SalesMuted);lead.followUpAt?.let{Text("Next follow-up ${friendlyLeadDate(it)}",style=MaterialTheme.typography.bodySmall,color=SalesMuted)};Row(horizontalArrangement=Arrangement.spacedBy(8.dp)){OutlinedButton(onFollowUp,enabled=!busy,modifier=Modifier.weight(1f)){Text("Add Follow-up")};Button(onCheckIn,enabled=!busy&&lead.stage !in listOf(LeadStage.WON,LeadStage.LOST),modifier=Modifier.weight(1f)){Text("Add Check-in")}};OutlinedButton(onTransition,enabled=!busy&&lead.stage !in listOf(LeadStage.WON,LeadStage.LOST),modifier=Modifier.fillMaxWidth()){Text("Stage / Next step")}}}}
