...
@Composable fun LeadsScreen(initialLeadId:String?=null,onInitialLeadConsumed:()->Unit={},onCheckIn:(LeadSummary)->Unit={},vm:LeadsViewModel=viewModel()){
 ...
 LazyColumn(Modifier.fillMaxSize().padding(horizontal=16.dp),contentPadding=PaddingValues(vertical=14.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){
  item{Text("Leads",style=MaterialTheme.typography.headlineSmall,fontWeight=FontWeight.Bold,color=SalesInk);Text("Manage your assigned leads and move them through the pipeline.",color=SalesMuted);Spacer(Modifier.height(8.dp));OutlinedTextField(search,{search=it},Modifier.fillMaxWidth(),label={Text("Search name or phone")},singleLine=true,leadingIcon={Icon(Icons.Default.Search,null)});Spacer(Modifier.height(8.dp));LazyRow(horizontalArrangement=Arrangement.spacedBy(6.dp)){item{FilterChip(stage==null,{stage=null},{Text("All Leads ${state.leads.size}")})};items(LeadStage.entries){s->val count=state.leads.count{it.stage==s};FilterChip(stage==s,{stage=s},{Text("${stageLabel(s)} $count")})}};state.message?.let{MessageBanner(it,vm::clear)}}
