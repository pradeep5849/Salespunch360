...
data class FollowUpsState(val loading:Boolean=true,val filter:String="TODAY",val tasks:List<FollowUpTask> = emptyList(),val message:String?=null,val busyTaskId:String?=null)
class FollowUpsViewModel(app:Application):AndroidViewModel(app){
 ...
 fun cancel(task:FollowUpTask)=viewModelScope.launch{_state.value=_state.value.copy(busyTaskId=task.id,message=null);try{api.cancelFollowUp(task.id);val result=api.followUps(_state.value.filter);_state.value=FollowUpsState(false,result.status,result.tasks,message="Follow-up cancelled.")}catch(e:Exception){_state.value=_state.value.copy(busyTaskId=null,message=apiMessage(e,"Follow-up couldn't be cancelled."))}}
