...
data class FieldState(val loading:Boolean=true,val context:FieldContext?=null,val leads:List<LeadSummary> = emptyList(),val followUps:List<FollowUpTask> = emptyList(),val busy:Boolean=false,val message:String?=null)
class FieldViewModel(app:Application):AndroidViewModel(app){private val session=SecureSession(app);private val api=ApiClient(session);private val _state=MutableStateFlow(FieldState());val state:StateFlow<FieldState> = _state;init{refresh()}
 private suspend fun scheduledVisitFollowUps()=listOf("OVERDUE","TODAY","PENDING").flatMap{api.followUps(it).tasks}.filter{it.canStartCheckIn}.distinctBy{it.id}.sortedBy{it.dueDate}
 fun refresh()=viewModelScope.launch{_state.value=_state.value.copy(loading=true);_state.value=try{FieldState(loading=false,context=api.fieldContext(),leads=api.leads().filter{it.assignedUserId==session.userId()},followUps=scheduledVisitFollowUps())}catch(_:Exception){_state.value.copy(loading=false,message="Field information couldn't be loaded.")}}
 ...
 fun checkout(visitId:String,location:LocationPayload,sentiment:VisitSentiment,remarks:String?,onSuccess:()->Unit={}){mutate("Checkout completed.",onSuccess){api.checkout(visitId,location,sentiment,remarks)}}
