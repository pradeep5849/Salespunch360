...
@OptIn(ExperimentalMaterial3Api::class) @Composable fun AuthenticatedApp(data:Bootstrap,message:String?,dismiss:()->Unit,attendance:(Boolean,LocationPayload,()->Unit)->Unit,logout:()->Unit){
 ...
 Scaffold(topBar={TopAppBar(title={CompanyIdentity(data.company.name)},actions={StatusChip(data.entitlement.state)})},bottomBar={NavigationBar{destinations.forEachIndexed{i,item->NavigationBarItem(selected=selected==i,onClick={selected=i},icon={Icon(item.icon,item.label)},label={Text(item.label)})}}}){padding->Column(Modifier.padding(padding).fillMaxSize()){message?.let{MessageBanner(it,dismiss)};when{selected==0->HomeScreen(data,attendance);selected==1&&data.user.role==MobileRole.COMPANY_ADMIN->EmployeesScreen();selected==1&&data.user.role==MobileRole.SALES->FieldScreen();selected==2&&data.user.role==MobileRole.SALES->LeadsScreen();selected==2&&data.user.role!=MobileRole.SALES->ReportsScreen();selected==destinations.lastIndex->MoreScreen(data,logout){destination->when(destination){"Employees"->selected=1;"Reports"->selected=2}};else->FoundationScreen(destinations[selected].label)}}}
}

@Composable private fun HomeScreen(data:Bootstrap,attendance:(Boolean,LocationPayload,()->Unit)->Unit){LazyColumn(Modifier.fillMaxSize().padding(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){item{Text("Good to see you, ${data.user.name.substringBefore(' ')}",style=MaterialTheme.typography.headlineSmall);Text(homeIntro(data.user.role))};if(data.user.role!=MobileRole.COMPANY_ADMIN)item{AttendanceCard(data,attendance)};if(data.user.role!=MobileRole.COMPANY_ADMIN)item{SyncStatusCard(data.user.id)};if(data.user.role==MobileRole.COMPANY_ADMIN)item{SeatCard(data)};items(homeFoundations(data).size){ContentCard(homeFoundations(data)[it].first,homeFoundations(data)[it].second)}}}
...
@Composable private fun MoreScreen(data:Bootstrap,logout:()->Unit,navigate:(String)->Unit){var page by remember{mutableStateOf<String?>(null)};if(page!=null){Column(Modifier.fillMaxSize()){TextButton({page=null},Modifier.padding(horizontal=8.dp)){Text("← More")};Box(Modifier.weight(1f)){when(page){"Geofence"->GeofenceScreen();"Targets"->TargetsScreen(data.user.role);"Leads"->LeadsScreen();"Subscription"->SubscriptionScreen();"Settings"->SettingsScreen();else->AccountScreen(data,logout)}}};return};val items=when(data.user.role){MobileRole.COMPANY_ADMIN->listOf("Company","Employees","Geofence","Targets","Leads","Subscription","Settings","Account");MobileRole.MANAGER->listOf("Targets","Leads","Reports","Account");MobileRole.SALES->listOf("My Targets","Account")};LazyColumn(Modifier.fillMaxSize().padding(16.dp),verticalArrangement=Arrangement.spacedBy(8.dp)){item{Text("More",style=MaterialTheme.typography.headlineSmall);Text("${data.company.name} · ${data.user.name}")};items(items){item->OutlinedButton({when(item){"Employees","Reports"->navigate(item);"Targets","My Targets"->page="Targets";else->page=item}},Modifier.fillMaxWidth()){Text(item)}};item{OutlinedButton(logout,Modifier.fillMaxWidth()){Icon(Icons.AutoMirrored.Filled.Logout,null);Spacer(Modifier.width(8.dp));Text("Sign out")};Spacer(Modifier.height(20.dp));Text("Powered by SalesPunch360",style=MaterialTheme.typography.labelSmall)}}}
@Composable private fun AccountScreen(data:Bootstrap,logout:()->Unit){Column(Modifier.fillMaxSize().padding(16.dp),verticalArrangement=Arrangement.spacedBy(12.dp)){ContentCard(data.company.name,listOfNotNull(data.user.name,data.user.email,data.user.role.name.replace('_',' ')).joinToString(" · "));OutlinedButton(logout,Modifier.fillMaxWidth()){Icon(Icons.AutoMirrored.Filled.Logout,null);Spacer(Modifier.width(8.dp));Text("Sign out")}}}
