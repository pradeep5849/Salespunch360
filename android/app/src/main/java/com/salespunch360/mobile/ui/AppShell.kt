...
@OptIn(ExperimentalMaterial3Api::class) @Composable fun AuthenticatedApp(data:Bootstrap,message:String?,dismiss:()->Unit,attendance:(Boolean,LocationPayload,()->Unit)->Unit,logout:()->Unit){
 ...
 Scaffold(topBar={TopAppBar(title={CompanyIdentity(data.company.name)},actions={StatusChip(data.entitlement.state)})},bottomBar={NavigationBar{destinations.forEachIndexed{i,item->NavigationBarItem(selected=selected==i,onClick={selected=i},icon={Icon(item.icon,item.label)},label={Text(item.label)})}}}){padding->Column(Modifier.padding(padding).fillMaxSize()){message?.let{MessageBanner(it,dismiss)};when{selected==0->HomeScreen(data,attendance);selected==1&&data.user.role==MobileRole.COMPANY_ADMIN->EmployeesScreen();selected==1&&data.user.role==MobileRole.SALES->FieldScreen();selected==destinations.lastIndex->MoreScreen(data,logout);else->FoundationScreen(destinations[selected].label)}}}
}
