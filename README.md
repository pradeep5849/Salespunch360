...
Stage 10 adds fail-fast environment validation, canonical HTTPS production URL requirements, stronger production secrets, explicit proxy trust, restrictive production security headers, private authenticated caching, PostgreSQL-backed distributed rate limiting, browser-origin validation for sensitive mutations, a non-sensitive database readiness endpoint, safe error/not-found recovery, structured safe logging, and an accessible password visibility toggle. The shared rate-limit table requires the sole Stage 10 integrity migration; Stage 1–9 migrations remain unchanged.

Production deployment, backup/restore, rollback, security, browser support, external infrastructure requirements, and the honest real-phone field-test checklist are documented in [`docs/production.md`](docs/production.md). Historical tenant data remains authorization-scoped and read-only after entitlement expiry; safe attendance end and visit checkout remain available. No new business module, service worker, offline cache, native application, live payment gateway, or fake payment success was added.

## Android Stage A

The isolated native Kotlin/Compose application under `android/` establishes secure mobile authentication for Company Admin, Manager, and Sales, role-aware company-branded navigation, attendance controls, a visible location foreground service, and a bounded idempotent Room/WorkManager location queue. Super Admin remains web-only. Architecture, build steps, limitations, and the required physical-phone acceptance procedure are documented in [`docs/android-stage-a.md`](docs/android-stage-a.md).
