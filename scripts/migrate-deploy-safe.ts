...
function resolveMigration(mode: "applied" | "rolled-back") {
  ...
  if (result.status !== 0) {
    throw new Error(
      `Failed to mark ${TARGET_MIGRATION} as ${mode}.`,
    );
  }
}
...
async function repairA10RuntimeMigration() {
  ...
  // The failed migration references owner_financial_accounts(companyId, id).
  // id is already a PK, but PostgreSQL requires a matching unique key/index for
  // the exact composite FK target.
  ...
  if (!partial) {
    // If PostgreSQL rolled the failed migration back completely, keep the
    // prerequisite unique index and allow Prisma to rerun the original migration.
    await prisma.$disconnect();
    resolveMigration("rolled-back");
    return true;
  }

  // Prisma/PostgreSQL can leave earlier statements from a failed migration in
  // place. Complete only the constraints that occur at/after the failure point,
